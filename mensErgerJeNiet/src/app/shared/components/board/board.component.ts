...
/*    this.setupHomeBase(BoardComponent.TRACK_BASE_L, BoardComponent.TRACK_BASE_W);*/
  ...
  onPawnClick(pawn: Pawn) {
    if (this.board.latestDiceResult > 0 && pawn.player == this.playerWithTurn) {
      //hier nog fixen dat hij niet voorbij de start positie kan lopen, dus ook dat hij de home base in gaat?

      pawn.currentPositionIndex =
        (pawn.currentPositionIndex + this.board.latestDiceResult)
        % this.board.totalTrackLength;
      this.board.nextTurn();
  /*    let forcedNewPawn = this.board.latestDiceResult;
      if(this.playerWithTurn.numPawnsInPlay > 0
        && !(pawn.player.startPosition = null)){*/


        /*for(let i = 0; i < (this.playerWithTurn.numFreePawns - this.playerWithTurn.numPawnsInPlay); i++;*/
  ...
  onRollDice() {
    ...
    if (this.board.latestDiceResult == 6 && this.board.placeAFreePawnAtStartPosition(this.playerWithTurn)) {
          ...
          console.log("TESTTTTTTTTTT" + this.playerWithTurn.getFreePawn());
